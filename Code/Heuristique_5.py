...
# TRI DE LISTE PAR DURÉE DÉCROISSANTE
    ...
    if state == "trier_liste_activites" :
        ...
        for i in range (len(activites)) :
            ...
            for j in range (i+1, len(activites)):
                ...
                if duree[numero-1] > duree[iMax-1] :
            ...
            if (duree[iMax-1] > duree[activite_a_echanger-1]) :

                tmp = activites[i]
                activites[i] = activites[res]
                activites[res] = tmp
        
        print ("Liste activites triee par duree decroissante", activites)
