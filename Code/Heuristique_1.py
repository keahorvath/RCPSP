...
def placer_activites(i, nb_activites, state, activites, precedence, activites_finies, debut_activite, fin_activite, ressources, duree_max, duree, ressource_max, liste_ressource_disponible):
    ...
    # on appelle la fonction "condition_precedence_1" du programme "contraintes" pour voir si l'activite i possede une liste de precedence ou pas
    condition_1 = contraintes.condition_precedence_1(i, lecteur.donnees.precedence)

    # L'"etat 1" nous permet de placer les activites. 
    if state == "first" :

        # on parcourt la liste "activites", en suivant l'ordre croissant de leurs indices
        for i in activites:

            # on verifie si l'activite n'a pas ete placee et si la contrainte de precedence est respectee
            if (i != 0 and contraintes.condition_precedence_2(i, lecteur.donnees.precedence, activites_finies) == 1) :
                ...
                # t2 correspond au temps à la date de début de l'activité i, qui vérifie les conditions de précédence et de ressources
                ...
                # On modifie les listes où sont stockees les debuts et les fins de chaque activite lorsqu'elles sont placees
                ...
                # On modifie la liste des ressources disponibles en enlevant la quantité de ressources utilisées par l’activité i à la quantité initiale. 
                ...
                # on passe a "l'etat 2", qui va nous renvoyer a "l'etat 1" si la totalite des activites n'a pas ete placee
                state = "second"

            # si la condition de precedence n'est pas verifiee pour l'activite i, ou si elle a deja ete placee, on passe a l'activite suivante
            else :
                state = "second"

    # "L'etat 2" est un etat intermediaire qui nous permet de verifier a chaque fois si l'ensemble des activites a ete placé
    if state == "second" :

        # si l'ensemble des activites n'a pas ete place, on retourne a "l'etat 1"
        if len(activites_finies) != len(activites) :
            state = "first"

        # si toutes les activites ont ete placees, on peut passer a "l'etat 3" et afficher les resultats
        else :
            state = "end"
    ...
    if state == "end" :
        ...
        # on cherche le temps ou toutes les activites ont ete terminees
        # on parcourt la liste de fin des activites et on trouve l'element le plus elevee qui correspond a la date de fin du projet
        fin = 0
...
def main() :
    ...
    # On appelle la fonction "placer_activites" dans "l'etat 1", c'est-a-dire qu'on va essayer de placer l'activite n°1
    state = "first"
    placer_activites(i, nb_activites, state, activites, precedence, activites_finies, debut_activite, fin_activite, ressources, duree_max, duree, ressource_max, liste_ressource_disponible)
