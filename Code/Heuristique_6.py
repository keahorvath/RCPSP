...
# TRI DE LISTE PAR DURÉE CROISSANTE 
    ...
    if state == "trier_liste_activites" :
        ...
        for i in range (len(activites)) :
            ...
            for j in range (i+1, len(activites)):
                ...
                if duree[numero-1] < duree[iMin-1] :
            ...
            if (duree[iMin-1] < duree[activite_a_echanger-1]) :

                tmp = activites[i]
                activites[i] = activites[res]
                activites[res] = tmp
        
        print ("Liste activites triee par duree croissante", activites)
